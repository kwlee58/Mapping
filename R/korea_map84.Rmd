...
library(broom) #> `tidy()`
# getwd()
dsn <- "../data/CTPRVN_201703"   #> 시도 단위 지도 파일 
shp1 <- readOGR(dsn, 
...
## Simplified Versions (from [mapshper](https://mapshaper.org))
...
```{r, eval = TRUE}
#> "discrete value supplied to continuous scale" 에러 발생하는 데 inline 코드로 실행시키면 문제 없이 실행됨.
ggplot(data = map_df,  
       mapping = aes(x = long,
                     y = lat,
                     group = group)) +
...
```{r, eval = TRUE}
ggplot(data = shp_simp84_df,  #> map_simp 로 돌리면 "discrete value supplied to continuous scale" 에러가 발생하는 데 inline 코드로 실행시키면 문제 없이 실행됨.
       mapping = aes(x = long,
                     y = lat,
                     group = group)) +
  geom_polygon(fill = "white",
